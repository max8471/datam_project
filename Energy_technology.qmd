...
```
filtered_df <- subset(filtered_df, select = c(-Currency, -prout))
...
# Create filtered_gdp by subsetting gdp based on the common values in the "country" column
...
Merging the data NOT WORKING YET
```{r}
merged_data <- left_join(filtered_df, filtered_gdp, by = c("Country" = "Country Name"))

# Reshape the data using pivot_longer
merged_data <- merged_data %>%
  pivot_longer(cols = ends_with("x"), names_to = "Year", values_to = "GDP") %>%
  mutate(Technology = "GDP") %>%
  select(Country, Technology, Year, GDP)
```
