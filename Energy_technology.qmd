...
After establishing the consistent correlation between Gross Domestic Product (GDP) and Total Budget (public RD&D spending) across the years, we further investigated the dynamics at the extremes—1980 and 2002—by pinpointing the lowest and highest R-squared values.
...
ggplot(df1980, aes(x = GDP, y = `Total Budget`)) +
  ...
  labs(title = "Regression Analysis: GDP vs Total Budget (1980)",
       x = "GDP",
       y = "Total Budget") +
  theme_minimal()
```
...
ggplot(df2002, aes(x = GDP, y = `Total Budget`)) +
  geom_point() +
  geom_smooth(method = "lm", se = FALSE, color = "lightblue") +
  labs(title = "Regression Analysis: GDP vs Total Budget (2002)",
       x = "GDP",
       y = "Total Budget") +
  theme_minimal()
```
...
These analyses of extreme points serve as key snapshots in time, allowing us to witness the evolution of the correlation between economic indicators and public RD&D spending.

#Examining G7 Countries: Evolution of RD&D Spending and GDP Relationship

Focusing our lens on the Group of Seven (G7) countries, we conducted a regression analysis to scrutinize the relationship between Gross Domestic Product (GDP) and Total Budget allocated to Research, Development, and Deployment (RD&D) in the energy technology sector.
...
In the initial graph titled "Regression Analysis: GDP vs Total Budget," spanning the entire period from 1974 to 2022, we observe a nuanced trajectory. Notably, there is a conspicuous decline in the total budget allocated to RD&D during the 80s, persisting until the early 2000s, after which a linear increase becomes apparent. This observation suggests a dynamic interplay between economic factors and government investments in energy-related research over the decades.
...
What is interesting to see from this graph, other than the relationship, is the decline in total budget allocated to RD&D energy technology from governments during the 80s and until the 2000, when total budget started increasing again linearly.

```{r}
#| echo: false
#| message: false
ggplot(df_pivot_regress, aes(x = Year, y = `Total Budget`)) +
  geom_area() +
  labs(title = "Regression Analysis: GDP vs Total Budget",
       x = "GDP",
       y = "Total Budget") +
...
To delve deeper into this evolving relationship, we refined our analysis by narrowing the focus to the 1990s onwards. In the graph titled "Regression Analysis: GDP vs Total Budget (1990-2022)," we notice a compelling upward trend in the total budget for energy technology RD&D. The strong correlation is evident not only in the regression line but is also supported by an R-squared value of 0.76. This high R-squared value indicates that approximately 76% of the variability in the total budget for RD&D spending can be explained by changes in GDP during this specific period.
...
The pronounced correlation from the 1990s onwards underscores a robust connection between economic growth and increased government spending on energy-related research within the G7 nations
...
Next, let's narrow our focus to key technologies, investigating how G7 countries allocated their resources to foster advancements in energy technologies. This detailed analysis will provide a more granular understanding of the technological trajectories that accompanied the observed financial dynamics.
...
Recognizing the need to address potential distortions in our analysis caused by variations in the Gross Domestic Product (GDP) across G7 countries, we took a crucial step to normalize our spending values. This normalization was achieved by expressing the spending figures as permillages of the GDP, providing a more standardized measure and enabling a comparative analysis that considers each country's economic context.
...
To underscore the significance of our spending normalization, we present a series of graphs comparing public Research, Development, and Deployment (RD&D) spending across G7 countries in different technologies. These visualizations showcase spending expressed as both a permillage of the Gross Domestic Product (GDP) and in 2022 USD. 
...
We see that for quite every technology, the spending in public rd&d, when expressed as a per millage of the GDP, is very similar and follows the same trend from one G7 country to another. And even more on the period 1990-2022, where the correlation between GDP and Total budget has been highlighted precedently. This will allow us to analyze the trend in spending at the G7 level while staying relevant, since this group is homogeneous enough.
...
In the first graph that includes nuclear spending, we observe variations in spending levels, with nuclear standing out as having a higher magnitude compared to other technologies. To provide a clearer picture of trends in the remaining technologies, a second graph excludes nuclear spending.
...
The ability to discern these trends at the G7 level, while staying relevant and considering the homogeneous nature of this group, provides valuable insights into the collective priorities and investments in energy technology RD&D.
...
In our exploration of the Research, Development, and Deployment (RD&D) spending landscape within G7 countries, we now turn our attention to how the percentage of spending on each energy technology has evolved relative to the Total Budget over the past decades. This analysis provides insights into the shifting priorities and investments in different technologies within the G7 consortium.
...
Hydrogen and fuel cells technology, while starting from virtually 0% in the 1980s and 1990s, has seen an increase. It reached around 3-5% of the total budget in the 2016-2022 period, showcasing a growing interest in these technologies, albeit from a relatively small base.
