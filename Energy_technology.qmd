...
### Filtering the data (IEA) {.appendix}
...
### Processing the GDP data to make it correctly mergeable with the IEA one {.appendix}
...
### Merging the two datasets {.appendix}
...
**Research Question:** The primary research question guiding this investigation is: "Have G7 countries experienced noticeable shifts in their energy technology landscape over the last 50 years, and how does this relate to the proportion of public RD&D spending in relation to their GDP?"

The choice of a group such as G7 for this analysis stems from its global economic significance and commitment to addressing energy challenges. By delving into the energy technology trajectories of these nations, we aim to identify trends that may offer insights into the changes in economic structures and innovation ecosystems. Additionally, linking these technological advancements to the proportion of GDP allocated to public RD&D allows to make them comparable from one country to another.
...
As an introduction, here you will find the evolution in RD&D spending in 3 major technologies (Nuclear power, fossil fuels and renewable energies) for 3 main areas (Europe, Asia and Oceania, Americas)
...
plot_Europe <- ggplot(reshaped_IEA_Europe, aes(x = Year, y = Value, color = Technology)) +
  geom_line() +
...
plot_Asia_Oceania <- ggplot(reshaped_IEA_Asia_Oceania, aes(x = Year, y = Value, color = Technology)) +
  geom_line() +
    scale_color_manual(values = c("Renewables" = "darkgreen", "Fossil fuels" = "brown", "Nuclear" = "purple"))+
...
plot_Americas <- ggplot(reshaped_IEA_Americas, aes(x = Year, y = Value, color = Technology)) +
  geom_line() +
  scale_color_manual(values = c("Renewables" = "darkgreen", "Fossil fuels" = "brown", "Nuclear" = "purple"))+
...
combined_plots <- plot_Europe / plot_Asia_Oceania / plot_Americas
combined_plots
```
...
#### Information about the initial IEA data file
```{r}
#| message: false
#| echo: false
n_rows_IEA=nrow(df)
...
#### Information about the initial GDP data
...
Indeed, we see that this proportion is growing very fast from 1974 until 1980, reaching .45%, before drastically diminishing to .05% around 1995. Since then, the levels are quite constant.
...
plot1nuke<-ggplot(filtered_data_G7_nuclear, aes(x = Year, y = Value_normalized, group=Country, color=Country)) +
  ...
  labs(title = "G7 - Nuclear",
       ...
       y = "%o of GDP",
       color = "Country")  +
  theme(axis.title = element_text(size = 7), title = element_text(size=7))
...
combined_plot_tot <- plot1tot + plot2tot + plot_layout(guides = "collect")&
  theme(legend.position = "bottom", legend.key.size = unit(0.2, "cm"), legend.title = element_text(8))
#combined_plot_tot
```
...
all_combined <- combined_plot_nuke / combined_plot_renewables / combined_plot_nrgeff / combined_plot_foss +
  plot_layout(heights = heights)
all_combined2 <- combined_plot_hydro / combined_plot_crosscut / combined_plot_pow / combined_plot_tot  +
  plot_layout(heights = heights2)
all_combined
all_combined2
```
...
In this final phase of our data exploration journey, we delve into the dynamic landscape of Research, Development, and Deployment (RD&D) spending across G7 countries. Our focus now shifts towards understanding how the percentage distribution of spending in each energy technology has evolved relative to the Total Budget over the last few decades. Through careful analysis and visualization, we aim to unveil pivotal insights into the changing priorities of G7 nations in the realm of energy technology innovation.
...
# Conclusion
Our journey spanning five decades of data analysis, exploration, and reflection has provided valuable insights into the complex interplay between economic development and energy technology evolution within the G7 consortium. Rooted in the quest to understand shifts in Research, Development, and Deployment (RD&D) spending, our exploration has unraveled key patterns, priorities, and correlations that define the trajectory of G7 nations.
At its core, this investigation sought to answer a pivotal research question: "Has there been a discernible change in the energy technology landscape among G7 countries over the last 50 years, and how does this relate to the proportion of public RD&D spending in relation to their Gross Domestic Product (GDP)?" Through meticulous analysis and visualization, we've navigated through nuanced trends, highlighting the dynamic nature of G7 commitments to energy technology.
Our journey commenced with an examination of the correlation between GDP and RD&D spending. The consistent and robust relationship underscored the link between economic prosperity and the allocation of resources to energy-related research. As we delved deeper into the G7 dynamics, our analyses unveiled a steady commitment to energy technology advancements, with occasional fluctuations marking broader economic and political shifts.
Recognizing the need for comparability across countries, we normalized spending values as a permillage of GDP. This transformation facilitated a standardized approach, enabling a more accurate representation of shared trends among G7 nations.
Our exploration of spending distribution across different energy technologies uncovers dynamic shifts in priorities. The decline in nuclear spending suggests a reassessment, potentially driven by safety concerns and a pursuit of sustainable alternatives. The ascent of energy efficiency reflects a shared commitment to mitigating environmental impact, while the rise of hydrogen and fuel cells hints at anticipation for future energy landscapes. The surge in renewable spending signifies a collective push towards greener, more sustainable energy solutions.
As we conclude this exploration, the implications of our findings extend beyond the realm of data and analysis. The trends observed—whether in the decline of nuclear spending, the rise of energy efficiency, or the emergence of sustainable alternatives—resonate with global imperatives for environmental stewardship and sustainable development. These insights raise questions that demand further exploration and lay the groundwork for future discussions on policy shifts, collaborative efforts, and innovations.
In conclusion, our findings transcend data points and graphs; they paint a narrative of resilience, adaptation, and collaborative strides made by G7 nations in the realm of energy technology. As we navigate the complexities of global imperatives for environmental stewardship and sustainable development, our insights serve as a reflection and a catalyst.
Looking forward, our analysis propels ongoing discussions, decisions, and actions that will shape the future of energy landscapes. In navigating the evolving frontier of energy technology investments, this exploration is not just a conclusion but a continuation—a call to navigate the energy frontier with resilience, innovation, and a shared commitment to a sustainable future.

# Other conclusion proposal

We've delved into data analysis, exploration, and reflection to gain valuable insights into how economic development and energy technology have evolved among the G7 nations over the past five decades. Our main focus was understanding changes in Research, Development, and Deployment (RD&D) spending and its connection to Gross Domestic Product (GDP). Our investigation aimed to answer a key question: "Have G7 countries experienced noticeable shifts in their energy technology landscape over the last 50 years, and how does this relate to the proportion of public RD&D spending in relation to their GDP?"

Through careful analysis and visualization, we've uncovered nuanced trends, emphasizing the dynamic nature of G7 commitments to energy technology. We began by examining the correlation between GDP and RD&D spending, finding a consistent and robust relationship highlighting the link between economic prosperity and investment in energy-related research. Further exploration revealed a steady commitment to energy technology advancements, with occasional fluctuations aligning with broader economic and political changes.

To ensure comparability across countries, we normalized spending values as a percentage of GDP, facilitating a standardized approach for a more accurate representation of shared trends. Our examination of spending distribution across different energy technologies revealed dynamic shifts in priorities. The decline in nuclear spending suggested a reevaluation, potentially influenced by safety concerns and a pursuit of sustainable alternatives. The emphasis on energy efficiency indicated a collective commitment to reducing environmental impact, while increased investment in hydrogen and fuel cells hinted at anticipation for future energy landscapes. The uptick in renewable spending showcased a joint effort towards greener, more sustainable energy solutions.

Beyond data and analysis, our findings have broader implications. The observed trends resonate with global imperatives for environmental stewardship and sustainable development. They raise questions that warrant further exploration and provide a foundation for future discussions on policy shifts, collaborative efforts, and innovations. In conclusion, our insights go beyond data points and graphs; they tell a story of resilience, adaptation, and collaborative progress by G7 nations in the realm of energy technology. As we navigate global imperatives for environmental stewardship and sustainable development, our insights serve as both reflection and catalyst. Looking ahead, our analysis propels ongoing discussions, decisions, and actions that will shape the future of energy landscapes. This exploration is not merely a conclusion but a continuation—a call to navigate the energy frontier with resilience, innovation, and a shared commitment to a sustainable future.